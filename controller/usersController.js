...
class Controller {
    ...
    static listAll(req, res) {
        ...
        if (req.session.user.role == 'Surveyor' || req.session.user.role == 'Dewan') {
    ...
    static async insert(req, res) {
        ...
        let dewan = await sq.query("SELECT * FROM `dewans` where deletedAt IS NULL ", { type: QueryTypes.SELECT });
        res.render('content-backoffice/user/insert', { kec, user: req.session.user, dewan });
    }
    ...
    static async edit(req, res) {
        ...
        usersModel.findAll({
            ...
            .then(async (respon) => {
                ...
                res.render('content-backoffice/user/edit', { respon, kec, user: req.session.user, dewan });
            })
            .catch(err => {
                res.json(err)
            })
    ...
    static submit_insert(req, res) {
        const { username, password, nama, role, dewanId, kecamatan, kelurahan } = req.body
        let encryptedPassword = bcrypt.hashPassword(password)
        usersModel.findAll({
            where: {
                username: username
            ...
            if (data.length) {
                ...
                let k = { username: username, password: encryptedPassword, nama: nama, role: role };
                if(dewanId){
                    k.dewanId = dewanId
                }
                usersModel.create(k, { returning: true }).then(respon => {
